...
// CODE HERE
const totalPopulation = populations.reduce((runningTotal, curElement) => runningTotal + curElement, 0);
...
const ordersAfterTax = orders.map(orders => orders.price + orders.price * orders.tax);
...
const purchasesSum = purchases.filter(purchases => purchases.owner === `Bob`).reduce((total,purchase) => total + purchase.price, 0);
console.log(purchasesSum);